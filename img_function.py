...
class CardPredictor:
    ...
    def train_svm(self):
        ...
        svmchinese_path = os.path.join(BASE_DIR, "svmchinese.dat")
        train_chars2 = os.path.join(BASE_DIR, "train", "chars2")
        train_charsChinese = os.path.join(BASE_DIR, "train", "charsChinese")
        ...
        if os.path.exists(svm_path):
            ...
            for root, dirs, files in os.walk(train_chars2):
        ...
        if os.path.exists(svmchinese_path):
            self.modelchinese.load(svmchinese_path)
        else:
            chars_train = []
            chars_label = []
            for root, dirs, files in os.walk(train_charsChinese):
                if not os.path.basename(root).startswith("zh_"):
                    continue
                pinyin = os.path.basename(root)
                index = img_recognition.provinces.index(pinyin) + PROVINCE_START + 1
                for filename in files:
                    filepath = os.path.join(root, filename)
                    digit_img = cv2.imread(filepath)
                    digit_img = cv2.cvtColor(digit_img, cv2.COLOR_BGR2GRAY)
    ...
    def save_traindata(self):
        ...
        if not os.path.exists(svmchinese_path):
            self.modelchinese.save(svmchinese_path)
        ...
        for i, color in enumerate(colors):
            if color in ("blue", "yello", "green"):
                ...
                for i, part_card in enumerate(part_cards):
                    ...
                    part_card = img_recognition.deskew(part_card)
